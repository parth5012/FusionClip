...
- **Explicit Celery Task ID on Pre-created DB Rows**: Pre-creating a `Task` row in DB before Celery dispatch (e.g. `upscale_<token>`) requires using `apply_async(..., task_id=task_id)`. Using `.delay()` generates a new random UUID for Celery, causing signal boundary events (`task_prerun`, `task_failure`) to write to an orphan row unlinked from the UI query.
